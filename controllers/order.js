...
exports.create = (req, res) => {
    ...
    order.save((error, data) => {
        ...
        // order.amount
        const emailData = {
            to: 'frankerr9189@gmail.com', // admin
...
// };
//guest shipping
exports.guestShipping = (req, res) => {
    console.log("req.body guest checkout", req.body);
    const guest = new Shipping(req.body);
    guest.save((err, guest) => {
        if(err){
            return res.status(400).json({
               // err: errorHandler(err)
               error: 'Guest Checkout Error'
            });
        }
        res.json({
            guest
